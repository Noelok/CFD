...
LRESULT CALLBACK WndProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	// ------------------------------------------
		...
		// --- 2. PANNING (Right Click) ---
			...
			camera.pos.x -= speed * ((float)dx * camera.R.xx + (float)dy * camera.R.yx);
			camera.pos.y -= speed * ((float)dx * camera.R.xy + (float)dy * camera.R.yy);
			camera.pos.z -= speed * ((float)dx * camera.R.xz + (float)dy * camera.R.yz);
