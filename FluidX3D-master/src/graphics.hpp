...
public:
  ...
  float zoom = 0.5f * (float)min(width, height),
        dis = 0.5f * (float)width /
              tan(fov * pif /
                  360.0f);     // zoom, distance from camera to rotation center
  ...
  double free_camera_velocity =
      1.0;                     // free camera speed; default: 1 cell per second
  bool vr = false, tv = false; // virtual reality mode (enables stereoscopic
                               // rendering), VR TV mode
  float eye_distance = 8.0f;   // distance between cameras in VR mode
  bool autorotation = false;   // autorotation
  bool lockmouse =
      false; // mouse movement won't change camera view when this is true
  std::atomic_bool key_update = true; // a key variable has been updated
  std::atomic_bool allow_rendering =
      false;                              // allows interactive redering if true
  std::atomic_bool allow_labeling = true; // allows drawing label if true
  std::mutex
      rendring_frame; // a frame for interactive graphics is currently rendered

private:
  float log_zoom = 4.0f * log(zoom), target_log_zoom = log_zoom;
  double mouse_x = 0.0, mouse_y = 0.0, target_mouse_x = 0.0,
         target_mouse_y = 0.0;    // mouse position
...
public:
  ...
  void update_matrix() {
    ...
    const float sinrx = sin((float)rx), cosrx = cos((float)rx),
                sinry = sin((float)ry), cosry = cos((float)ry);
    R.xx = cosrx;
    R.xy = sinrx;
    R.xz = 0.0f;
    R.yx = sinrx * sinry;
  ...
  void input_key(const int key) {
    ...
    if (!lockmouse) {
      if (key == 'I')
        input_I(); // rotating camera with keys
      if (key == 'J')
        input_J();
      if (key == 'K')
        input_K();
    ...
    if (key == 'Y')
      input_Y(); // adjusting field of view
    if (key == 'X')
      input_X();
    if (key == 'N')
      input_N(); // adjust camera.vr eye distance
    if (key == 'M')
      input_M();
  ...
  void update_state(const double frametime) {
    if (!free) {
      log_zoom = (float)exp_decay((double)log_zoom, (double)target_log_zoom,
                                  frametime, 0.083); // smoothed zoom
      ...
      if (get_key_state('D'))
        input_D(frametime);
      if (get_key_state(' '))
        input_Space(frametime);
      if (get_key_state('C'))
        input_C(frametime);
    }
    if (!lockmouse) {
      if (get_key_state('I'))
        input_I(frametime); // rotate camera with keys
      if (get_key_state('J'))
        input_J(frametime);
      if (get_key_state('K'))
        input_K(frametime);
    ...
    if (autorotation)
      update_rotation(-45.0 * frametime, 0.0); // 45 degrees per second
    if (get_key_state('Y'))
      input_Y(); // adjust field of view
    if (get_key_state('X'))
      input_X();
    if (get_key_state('N'))
      input_N(); // adjust vr eye distance
    if (get_key_state('M'))
      input_M();
    if (!lockmouse) {
      mouse_x = exp_decay(mouse_x, target_mouse_x, frametime,
                          0.031); // smoothed mouse movement
  ...
  void clear_frame() {
    std::fill(bitmap, bitmap + width * height,
              GRAPHICS_BACKGROUND_COLOR); // faster than "for(uint i=0u;
                                          // i<width*height; i++) bitmap[i] =
                                          // GRAPHICS_BACKGROUND_COLOR;"
    std::fill(zbuffer, zbuffer + width * height,
              min_int); // faster than "for(uint i=0u; i<width*height; i++)
                        // zbuffer[i] = min_int;"
  }
  float
  data(const uint i) const { // returns all camera data required for rendering
    switch (i) {
    case 0:
      return zoom; // camera zoom
    case 1:
      return dis; // distance from camera to rotation center
    case 2:
      return free ? pos.x : 0.0f; // camera position
    case 3:
      return free ? pos.y : 0.0f;
    case 4:
      return free ? pos.z : 0.0f;
    case 5:
      return R.xx; // camera rotation matrix
    case 6:
      return R.xy;
    case 7:
      return R.xz;
    ...
    case 13:
      return R.zz;
    case 14:
      return as_float((uint)vr << 31 | (uint)tv << 30 |
                      ((uint)float_to_half(eye_distance) &
                       0xFFFF)); // stereoscopic rendering parameters
    default:
      return 0.0f;
    }
  }
...
private:
  ...
  void input_R() { autorotation = !autorotation; }
  void input_U() { lockmouse = !lockmouse; }
  ...
  void input_N() { eye_distance = fmax(eye_distance - 0.2f, 0.0f); }
  void input_M() { eye_distance += 0.2f; }
  ...
  double exp_decay(const double a, const double b, const double frametime,
                   const double halflife = 1.0) {
    return b + (a - b) * exp2(-frametime / halflife);
  }
};

extern Camera camera;
extern bool key_E, key_G, key_H, key_O, key_P, key_Q, key_T,
    key_Z; // defined in graphics.cpp
extern bool key_1, key_2, key_3, key_4, key_5, key_6, key_7, key_8, key_9,
    key_0; // defined in graphics.cpp

#define GRAPHICS_CONSOLE // open console additionally to graphics window
#define FONT_HEIGHT 22   // default: 11
#define FONT_WIDTH 12    // default: 6
...
void draw_triangle(const float3 &p0, const float3 &p1, const float3 &p2,
                   const int color, const bool translucent = false);
void draw_triangle(const float3 &p0, const float3 &p1, const float3 &p2,
